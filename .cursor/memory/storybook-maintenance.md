...
- **Sidebar = filesystem under `patterns/`**: `main.ts` uses a `stories` entry with `directory` → `packages/ui/src/patterns` (no `title` in `*.stories.tsx`; order via `preview.ts` `storySort`). See `.cursor/rules/storybook.mdc`. Storybook may **hoist** (collapse) when a single story’s filename matches the parent folder after normalization—that’s the default and is **acceptable**; use natural `ComponentName.stories.tsx` names unless you intentionally want an extra sidebar row.
- **Canvas parity with web**: `preview.ts` may still import `packages/ui/src/styles/index.scss` or `patterns/globals.scss` during migration — **`styles/` is deprecated** (see `readboot-scss-architecture.mdc`); align Storybook with **`framework/`** once the package export exists. **Fonts** also need the same Google Fonts `<link>`s as `apps/web/app/layout.tsx` — those go in `.storybook/preview-head.html` (Storybook does not use Next’s `<head>`). **Manager chrome** (sidebar logo/title): `.storybook/manager.ts` + `apps/storybook/public/branding/readboot-logo.png` (see `staticDirs`). Tab icon: `public/favicon.png` + `preview-head.html` `<link rel="icon">`.
