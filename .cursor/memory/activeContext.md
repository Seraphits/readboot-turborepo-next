...
- Recent: **Web blog routes** — `/blog/`, `/blog/category/[slug]/`, `/blog/post/[slug]/` use **`ShowcaseTemplate`** + boxed **`ReadBootBand`** (optional CTA) + **`BlogCategoryNav`** + **`BlogShowcase`** / **`BlogArticleLayout`** + `WPContent`; copy in **`apps/web/content/blogPage.ts`**; **`BlogCard`** CTA label from **`blogCardDefaults`**.
